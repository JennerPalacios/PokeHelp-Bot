	...
	async execute(timeStamp,timeStampEmbed,cc,message,sid,botGuilds,botChannels,botUsers,botConfig,serverSettings,globalSettings,discordVersion,processVersion){
		...
		let embedMSG={
			"embed": {
				...
				"description": "```md\n"+botConfig.cmdPrefix+"kick <@mention/id> [reason]```"
			}
		};
		...
		if(member.roles.has(modRole.id) || member.roles.has(adminRole.id) || member.id===botConfig.ownerID){
			if(args.length<1){
				...
				if(!mentionMember.id){
					return channel.send("⚠ Please `@mention` a person you want me to **kick**, "+member)
					...
					if(sendToChannelTyped==="yes"){
						channel.send("⚠ "+mentionMember+" has been **kicked**! 💪",{embed:{color:0xFFAA11,description:reasons}})
							.catch(err=>console.info(timeStamp+" "+cc.hlred+" ERROR "+cc.reset+" "+err.message))
					}
					console.log(timeStamp+" "+cc.lblue+mentionMember.user.username+cc.reset+"("+cc.cyan+mentionMember.id+cc.reset
						+") has been "+cc.green+"kicked"+cc.reset+" from server: "+cc.purple+guild.name+cc.reset+", by: "+cc.red+member.user.username+cc.reset+"("+member.id+")");
					return guild.member(mentionMember.id).kick(reasons).catch(err=>console.info(timeStamp+" "+cc.hlred+" ERROR "+cc.reset+" "+err.message))
				}
			}
		}
	}

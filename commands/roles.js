module.exports={
	...
	execute(timeStamp,timeStampEmbed,cc,message,sid,botGuilds,botChannels,botConfig,serverSettings,globalSettings,discordVersion,processVersion){
		...
		let args=message.content.toLowerCase().split(/ +/).slice(1),ARGS=message.content.split(/ +/).slice(1),
			mentionMember={},mentionChannel={},guild=message.guild,channel=message.channel,member=message.member;
		...
		let embedMSG={
			"embed": {
				...
				"description": "```md\n"+botConfig.cmdPrefix+"roles count\n"
					...
					+botConfig.cmdPrefix+"role <@mention> <roleName>\n"
					+botConfig.cmdPrefix+"role remove <@mention> <roleName>```"
			}
		};
